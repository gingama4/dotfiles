...
function M.cmp_source(name, icon)
  icon = icon or GinVim.config.icons.kinds[name:sub(1, 1):upper() .. name:sub(2)]
  local started = false
...
function M.pretty_path(opts)
  ...
  return function(self)
    ...
    path = GinVim.norm(path)
    local root = GinVim.root.get({ normalize = true })
    local cwd = GinVim.root.cwd()
